...
#### LOAD FILE USED IN CLUSTERING!!
  data_used <- read_csv('data/ASD_ADHD_only_regional_data.csv')
results_dir <- "results/sept_results/ASD_ADHD_only_regional_all_sept/"
...
#combined_data <- combined_data%>%
#   select(c(ID, dx.original, group, sex, cluster_assignment, GMVTransformed.q.wre, WMVTransformed.q.wre))
...
# 
#     #If p value is significant, include 'significant' directory
#     if (!is.na(stat_test$p.value) & stat_test$p.value < 0.01){
#       saving_dir <- file.path(cluster_dir, "sig")
#     } else{
#       saving_dir <- cluster_dir
#     }
...
features_for_table <- c('age', 'sex', 'IQ', 'RBSR6_TOTAL', 'ADOS_CSS', 'SRS_TOTAL_RAW', 'SWAN_ADHD_I_SUB', 'SWAN_ADHD_HI_SUB', 'FSQC', 'site', 'dx.original')
...
#write_csv(table_df, file.path(main_dir, 'table2_KW001_test.csv'))
